...
const toolSchemas = [
  ...
  { name: 'execute_action_draft', description: "U5: controlled execution of an Agent Wallets Console action draft (x402-action-draft-v1, kind='send' only in this phase). Re-resolves the caller's agent/wallet/budget fresh at execution time -- never trusts the draft's cached resolution -- and rejects if the current wallet no longer matches the draft's declared source_wallet_id. draft_id is the sole idempotency key: a retried call with the same draft_id returns the original terminal result instead of executing a second transfer. Delegates all signing/submission to circle_gasless_transfer; introduces no new wallet authority.",
