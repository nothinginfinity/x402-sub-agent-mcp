...
**Migration note (2026-07-29):** The `agent_context` tables from `migrations/0006_agent_context.sql` are NOT self-sufficient. The deployed `worker.js` `provisionAgentContextSelf` handler was updated after 0006 and requires two follow-up migrations that were applied to live D1 after the fact: `0007_agent_wallet_id.sql` (adds the `wallet_id` column + partial unique index on `agent_wallets(wallet_id, network, asset) WHERE status='active'`) and `0008_agent_context_unique_constraints.sql` (adds the unique constraints on `agent_permissions` and `agent_budgets` that the upsert `ON CONFLICT` clauses target). A rebuild from migrations MUST apply 0006, 0007, and 0008 together, in order, or `provision_agent_context_self` will fail with `no such column: wallet_id` and then `ON CONFLICT ... does not match any ... UNIQUE constraint`. Root cause: code shipped ahead of its migrations.
