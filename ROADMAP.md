...
- [ ] Phase 3 (full roster, hundreds): auto-provisioning and simple default per-tool pricing, since hand-registering hundreds of `payment_rules` doesn't scale. NOT started -- current roster is 6, not hundreds. This is the one piece of the original "Phase 3: scale to the full sub-agent roster" bullet still open; everything else that bullet referenced (budget-policy granularity) is done above.
