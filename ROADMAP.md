...
- [ ] Support budget policies per agent, organization, tool, route, time period, and maximum single transaction. NOT yet implemented -- only account-level (owner+asset+network) enforcement exists today. `route`/`tool`/`caller_id` are captured on each hold for observability but not yet enforced as separate limits. Deferred to V1.3B.5 Phase 3 below, where it becomes necessary at scale rather than before.
...
- [ ] Add peer-to-peer credit to x402-balance-ledger-mock. Currently `commit` only debits a payer -- there is no payee credit path. `reserve` needs a `payee_owner` (or `payee_account_id`) captured at hold-creation time, locked before `commit` so a payment's destination can't be redirected after the fact. `commit` needs to perform a real double-entry move (debit payer, credit payee) inside the same D1 transaction, not two separate mutations. Payee accounts auto-provision via the same get-or-create pattern `fund_test_account` already uses.
- [ ] Distinguish caller identity so spend/attribution can be split by driving LLM/session (e.g. a `caller_id` convention like `claude:<session>` vs `chatgpt:<session>`). Reuses the existing `caller_id` field already on holds -- no new column needed for this.
- [ ] Phase 1: prove the peer-to-peer path end-to-end with exactly 2 sub-agents -- agent A calls a tool gated behind x402 on agent B, A's balance decreases, B's balance increases by the same amount, the receipt shows both sides. Live-verify with real curl calls against the deployed Workers, not code review alone.
- [ ] Phase 2: expand to a handful of agents (3-6), including the Claude-vs-ChatGPT caller distinction from above.
- [ ] Phase 3: scale to the full sub-agent roster (hundreds). This is where auto-provisioning and simple default per-tool pricing matter, since hand-registering hundreds of `payment_rules` doesn't scale. Revisit the budget-policy granularity deferred from V1.3B here, once per-agent/per-tool spend limits become genuinely necessary at that scale -- not required for Phases 1-2.
