...
- [ ] Apply `oauth_capture_arms` migration to live D1 (manual, before deploy).
- [ ] Add `admin_bind_oauth_identity(agent_id, family)` tool (register + dispatch + `mcp:admin` gate + idempotent arm upsert + family-conflict guard + audit).
- [ ] Add passive capture-on-first-connect to `resolveAgentContext` (atomic arm-consume on the OAuth credential-miss branch, writing the family credential via `attachAgentCredential`, then re-resolve).
- [ ] Live-verify end-to-end: arm a throwaway family/agent, connect, confirm the credential is captured exactly once, the arm is consumed, resolve returns the bound agent, and a second connect is a no-op (no duplicate credential).
